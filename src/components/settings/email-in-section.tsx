...
export function EmailInSection() {
  ...
  async function handleApprove(id: string) {
    ...
    if (r.ok) {
      toast.success(t("settings.emailIn.imported"))
  ...
  return (
    <div className="px-4 py-3 rounded-lg border bg-card space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("settings.emailIn.cardTitle")}</h3>
        <SectionHelp guideId="markdownFormat" label={t("settings.emailIn.guideLabel")} className="ml-auto" />
      </div>
      ...
      <div className="rounded-md border border-primary/30 bg-primary/5 p-3 space-y-1.5">
        <p className="text-[10px] uppercase tracking-wider text-primary/70 font-medium">
          {t("settings.emailIn.yourAddress")}
        </p>
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          {t("settings.emailIn.addressHelp")}
        </p>
      </div>
              ...
              >
                <div className="flex items-start gap-2 mb-1.5">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium truncate">{e.subject || t("settings.emailIn.noSubject")}</p>
                    <p className="text-[10px] text-muted-foreground/80 truncate">
                      {t("settings.emailIn.from").replace("{sender}", e.fromName ? `${e.fromName} <${e.from}>` : e.from)}
                    </p>
                    {e.preview && (
