...
/**
 * Live clock pinned to the top-right of the viewport (above the page header).
 * Reads format prefs from `usePreferences().clock`. Updates every second when
 * seconds are shown, every minute otherwise.
 *
 * Render once near the top of <SidebarInset>; it positions itself with fixed
 * coords so it overlays page content without affecting layout.
 */
export function TopbarClock() {
  ...
  return (
    <div
      className="pointer-events-none fixed top-2 right-3 z-30 select-none flex flex-col items-end gap-1"
      aria-label="Current time"
      data-slot="topbar-clock"
    >
      <div className="group pointer-events-auto inline-flex items-stretch gap-0 rounded-lg border border-border/30 bg-background/50 backdrop-blur-md supports-[backdrop-filter]:bg-background/30 shadow-[0_1px_2px_rgb(0_0_0/0.04)] overflow-hidden transition-[background,border-color,box-shadow] duration-300 hover:bg-background/80 hover:border-border/60 hover:shadow-[0_4px_16px_-4px_rgb(0_0_0/0.15)]">
        <span className="flex items-center px-2.5 py-1 font-mono text-sm tabular-nums leading-none tracking-tight">{timeStr}</span>
        {dateStr && (
      ...
      {altStr && (
        <div className="pointer-events-auto inline-flex items-center gap-1.5 rounded-md bg-background/30 px-2 py-0.5 backdrop-blur text-[10px] text-muted-foreground/70 uppercase tracking-wider">
