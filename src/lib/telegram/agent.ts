...
function helpText(): string {
  return [
    "*FlowSpace bot — commands & settings*",
    "",
    "📥 *Capture*",
    ...
    "  • Forward any message → becomes a todo",
    "  • Send a 🔗 TikTok/YouTube/Instagram link → transcribed & saved",
    "  • Hold the 🎙 mic → voice note transcribed & captured",
    "  • Send a 🖼 photo → saved to your Gallery (auto-captioned)",
    "  _After a capture, buttons let you send it to a Page / Project / list._",
    "",
    "🪄 *Smart capture* (sprinkle in any message)",
    "  • `!urgent` `!high` `!medium` `!low` — priority",
    "  • `@2026-06-15` `@today` `@tomorrow` `@next-week` — due date",
    ...
    "  • `/default [name]` — show / set your default capture list",
    "  • `/caption [en|ar|auto]` — gallery AI-caption language",
    "  • `/voice [en|ar|auto]` — voice-transcription language",
    "  • `/voice skip on|off` — skip the per-voice language/destination pickers",
    "  • `/voiceout on|off` — hear my replies as voice notes (needs a TTS provider)",
    "  • `/nl on|off` — natural-language commands (needs an AI provider)",
    "  • `/digest [HH:MM|on|off]` — morning digest time/toggle",
    "  • `/clear` — wipe my saved message history",
    "  • `/me` — which account & bot you're connected to",
  ].join("\n")
}
